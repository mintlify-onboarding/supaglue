---
title: "Concepts"
description: "This section goes over the lifecycle of the developer and customer flow using Supaglue."
---
...
1. A [Developer Config](/concepts) is authored by developers to define a set of [Sync Configs](/concepts) each of which define the behavior of a [Sync](/concepts)
...
3. Customers opt-in and use Syncs by saving [Sync Values](/concepts) to the Supaglue Integration Service using [Supaglue React components](/react-components) which are embedded by developers into their applications
4. The Supaglue Integration Service executes Syncs, using the customer-provided Sync Values during runtime, as a [Sync Run](/concepts)
...
A Developer Config is a Typescript object that declaratively defines the behavior of a set of [Sync Configs](/concepts). It also contains metadata about Salesforce as a data source.
...
See the [Config SDK](/config-sdk) for a reference and [example](/config-sdk) Developer Config.

### Sync

A Sync Config is a Typescript object that declaratively defines the behavior of one [Sync](/concepts). One Sync Config defines the following:

1. Whether this sync is inbound or outbound
2. The standard object type to sync to/from Salesforce
3. How often it should sync
4. Where it should deliver the records (Webhook or Postgres or Salesforce)
5. How field mappings between Salesforce and the developer's application should work (i.e. which fields are exposed, how they are displayed, and their default field mappings)
6. Other operational configurations around retries, fetch strategies, and more

<Info>NOTE: A Developer Config is a set of Sync Configs. Your application will likely have one Developer Config, but several Sync Configs.</Info>
