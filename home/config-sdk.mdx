...
Use **Supaglue's Config SDK**, a Typescript-based configuration language, to define [Developer Configs](/concepts).
...
Entry-point for creating a Supaglue [Developer Config](/concepts): a set of [Sync Configs](/concepts) and Salesforce credentials metadata.
...
A Postgres [Internal Destination](/concepts).
...
If `customPropertiesColumn` is defined in your destination config, your customers will be able to create custom properties (by clicking the "+ Add custom property" button in the `SyncConfigCard`) and include them in their field mapping.

Important: The `customPropertiesColumn` must be the name of a JSONB column on the same Postgres table that represents the destination in your Supaglue config.

`internal.destinations.webhook(params: Params)`

A webhook [Internal Destination](/concepts).
...
A Postgres [Internal Source](/concepts).
...
A Salesforce [Customer Destination](/concepts). You can configure a specific Salesforce object to sync or allow the customer to choose from a list.
...
const objectConfig1 =
  sdk.customer.common.salesforce.specifiedObjectConfig('Contact');
const objectConfig2 = sdk.customer.common.salesforce.selectableObjectConfig([
  'Lead',
  'Contact',
]);
```
